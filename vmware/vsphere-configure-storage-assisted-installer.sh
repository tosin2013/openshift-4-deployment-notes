...
sed -i "s/server = .*/server =  ${SERVER}/g" cloud-provider-config.yaml
sed -i "s/datacenter = .*/datacenter =  ${DATACENTER}/g" cloud-provider-config.yaml
sed -i "s/default-datastore = .*/default-datastore =  ${DEFAULT_DATASTORE}/g" cloud-provider-config.yaml
sed -i "s|folder = .*|folder =  ${FOLDER_NAME}|g" cloud-provider-config.yaml
